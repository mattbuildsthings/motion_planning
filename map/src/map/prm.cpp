...
namespace map
{
	...
	void PRM::build_map(const int &n, int &k, const double &thresh)
	{
		...
		// Following PRM Algorithm from Lavalle - Planning Algorithms  page 253(237)
	...
	void PRM::sample_configurations(const int &n)
	{
		...
		int kill_counter = 0;
		while (static_cast<int>(configurations.size()) < n and kill_counter < 1000 * n)
		{
			// Sample random number with assigned limits
			std::uniform_real_distribution<double> dx(map_min.x, map_max.x); //uniform, not gaussian p(x|a,b) = 1/(b - a), a <= x <=b
			std::uniform_real_distribution<double> dy(map_min.y, map_max.y);
			// Random Number Generator defined in nuslam package
			double sample_x = dx(nuslam::get_random()); //static variable in ekf file
			...
			q.id = static_cast<int>(configurations.size()); //This cast would be unnecessary if you didn't use -1 for 'testing'
		...
		for (auto obs_iter = obstacles.begin(); obs_iter != obstacles.end(); obs_iter++)
		{
			...
			for (auto v_iter = obs_iter->vertices.begin(); v_iter != obs_iter->vertices.end(); v_iter++)
			{
				...
				// Only way this can be an obstacle now is if it's ON and edge
				{
					...
					if ((shrt.u >= 0.0 and shrt.u <= 1.0) or rigid2d::almost_equal(shrt.u, 0.0) or rigid2d::almost_equal(shrt.u, 1.0))
					{
				...
				else if (rigid2d::almost_equal(shrt.D, 0.0) and ((shrt.u >= 0.0 and shrt.u <= 1.0) or (rigid2d::almost_equal(shrt.u, 0.0) or rigid2d::almost_equal(shrt.u, 1.0))))
	...
	ShortestDistance lineToPoint(const Vertex &E1, const Vertex &E2, const Vertex &P0)
	{
		...
		shrt.u = ((P3(0) - P1(0)) * (P2(0) - P1(0)) + (P3(1) - P1(1)) * (P2(1) - P1(1))) / Eigen::Vector2d(P2(0) - P1(0), P2(1) - P1(1)).squaredNorm();
